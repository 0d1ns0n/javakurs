...
public class Main {
    ...
    private static double finPrice = 0;
    ...
    static void start() {
        ...
        while (true) {
            ...
            switch (drinkMachine) {
                case TEA -> {
                    System.out.printf((strS) + "%n", drinkMachine.getTitle(), DrinkPrice.Tea);
                    DrinkMaker.makeDrink(DrinksMachine.TEA);
                    finPrice += DrinkPrice.Tea;
                }
                case COFFEE -> {
                    System.out.printf((strS) + "%n", drinkMachine.getTitle(), DrinkPrice.Coffee);
                    DrinkMaker.makeDrink(DrinksMachine.COFFEE);
                    finPrice += DrinkPrice.Coffee;
                }
                case MOJITO -> {
                    System.out.printf((strS) + "%n", drinkMachine.getTitle(), DrinkPrice.Mojito);
                    DrinkMaker.makeDrink(DrinksMachine.MOJITO);
                    finPrice += DrinkPrice.Mojito;
                }
                case COCACOLA -> {
                    System.out.printf((strS) + "%n", drinkMachine.getTitle(), DrinkPrice.CocaCola);
                    DrinkMaker.makeDrink(DrinksMachine.COCACOLA);
                    finPrice += DrinkPrice.CocaCola;
                }
                case LEMONADE -> {
                    System.out.printf((strS) + "%n", drinkMachine.getTitle(), DrinkPrice.lemonade);
                    DrinkMaker.makeDrink(DrinksMachine.LEMONADE);
                    finPrice += DrinkPrice.lemonade;
                }
                case MINERALWATER -> {
                    System.out.printf((strS) + "%n", drinkMachine.getTitle(), DrinkPrice.MineralWater);
